...
void delay(int delay) {
	std::cout << "[SteeringWheel] sleep " << delay << "s" << std::endl;
	std::this_thread::sleep_for(std::chrono::seconds(delay));
}

void delay_ms(int delay) {
	// std::cout << "[SteeringWheel] sleep " << delay << "ms" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
}

// [todo]
// [example]: const char* socket_path = "/root/someip_app/tmp/uds_vehicle_speed";
const char* socket_path = "/tmp/uds/uds_steeringwheel";
int server_socket = -1;
...
void socket_thread() {
    ...
    while (running) {
        ...
        while (running) {
            ...
            ssize_t bytes_received = recv(client_socket, steeringwheel_data, sizeof(steeringwheel_data), 0);
            if (bytes_received <= 0) {
                break;
            }
			...
			if (count % 50 == 0) {
				std::stringstream ss;
				ss << "Received: " << steeringwheel_data[0];
				count = 0;
...
// todo
void send_steeringwheel_data(std::shared_ptr<SteeringWheelStubImpl> myService) {
    while (true) {
		// vehicle position
...
int main() {
    ...
    std::shared_ptr<SteeringWheelStubImpl> myService = std::make_shared<SteeringWheelStubImpl>();
    runtime->registerService("local", "test", myService);
    std::cout << "Successfully Registered Service!" << std::endl;

	// todo
	// sending thread
	std::thread th1(send_steeringwheel_data, myService);

    while (true) {
		std::cout << "[SteeringWheel] main thread .... time interval(1s)" << std::endl;
        delay(1);
    }
