...
#include "Collision_server_stubimpl.hpp"

 
using namespace std;

void delay(int delay) {
	std::cout << "[Collision] sleep " << delay << "s" << std::endl;
	std::this_thread::sleep_for(std::chrono::seconds(delay));
}

void delay_ms(int delay) {
	// std::cout << "[Collision] sleep " << delay << "ms" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
}

// [todo]
// [example]: const char* socket_path = "/root/someip_app/tmp/uds_vehicle_speed";
const char* socket_path = "/tmp/uds/uds_collision";
int server_socket = -1;
...
void socket_thread() {
    ...
    while (running) {
        ...
        while (running) {
            ...
            ssize_t bytes_received = recv(client_socket, collision_data, sizeof(collision_data), 0);
            if (bytes_received <= 0) {
                break;
            }
...
// todo
void send_collision_data(std::shared_ptr<CollisionStubImpl> myService) {
    while (true) {
		// vehicle position
		myService->fireObjectTypeEvent(collision_data[0]);
...
int main() {
    ...
    std::shared_ptr<CollisionStubImpl> myService = std::make_shared<CollisionStubImpl>();
    runtime->registerService("local", "test", myService);
    std::cout << "Successfully Registered Service!" << std::endl;

	// todo
	// sending thread
	std::thread th1(send_collision_data, myService);
