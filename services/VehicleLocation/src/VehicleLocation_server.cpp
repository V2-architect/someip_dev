...
void delay(int delay) {
	std::cout << "[VehicleLocation] sleep " << delay << "s" << std::endl;
	std::this_thread::sleep_for(std::chrono::seconds(delay));
}

void delay_ms(int delay) {
	// std::cout << "[VehicleLocation] sleep " << delay << "ms" << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(delay));
}

// [todo]
// [example]: const char* socket_path = "/root/someip_app/tmp/uds_vehicle_speed";
const char* socket_path = "/tmp/uds/uds_vehicle_location";
int server_socket = -1;
...
void socket_thread() {
    ...
    while (running) {
        ...
        while (running) {
            ...
            ssize_t bytes_received = recv(client_socket, vehicle_loc_data, sizeof(vehicle_loc_data), 0);
            if (bytes_received <= 0) {
                break;
            }
			...
			if (count % 50 == 0) {
				std::stringstream ss;
				ss << "Received: " << vehicle_loc_data[0] << " | " << vehicle_loc_data[1] << " | " << vehicle_loc_data[2];
				count = 0;
...
// todo
void send_vehicle_location(std::shared_ptr<VehicleLocationStubImpl> myService) {
    while (true) {
		// vehicle position
		myService->fireLocationInfoEvent(
...
int main() {
    ...
    std::shared_ptr<VehicleLocationStubImpl> myService = std::make_shared<VehicleLocationStubImpl>();
    runtime->registerService("local", "test", myService);
    std::cout << "Successfully Registered Service!" << std::endl;

	// todo
	// sending thread
	std::thread th1(send_vehicle_location, myService);

    while (true) {
		std::cout << "[VehicleLocation] main thread .... time interval(1s)" << std::endl;
        delay(1);
    }
