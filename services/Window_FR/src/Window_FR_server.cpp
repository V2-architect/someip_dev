...
#include "Window_FR_server_stubimpl.hpp"
...
void delay(int delay) {
	std::cout << "[Window_FR] sleep " << delay << "s" << std::endl;
	std::this_thread::sleep_for(std::chrono::seconds(delay));
}

void delay_ms(int delay) {
	// std::cout << "[Window_FR] sleep " << delay << "ms" << std::endl;
...
void changePosition(std::shared_ptr<Window_FRStubImpl> myService, int32_t& curr_pos, int32_t& new_pos) {
...
// todo
// position = 0~100
void updatePosition(std::shared_ptr<Window_FRStubImpl> myService) {
    ...
    while (true) {
		...
		std::cout << "[Window_FR] set_window_position_thread: " << curr_pos << std::endl;
		while (true) {
...
int main() {
    ...
    std::shared_ptr<Window_FRStubImpl> myService = std::make_shared<Window_FRStubImpl>();
    ...
    while (true) {
		std::cout << "[Window_FR] main thread .... time interval(1s)" << std::endl;
        delay(1);
    }
